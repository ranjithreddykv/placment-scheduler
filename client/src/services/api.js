...
export const datasetApi = {
  generate: (seed) =>
    api.post("/dataset/generate", seed ? { seed } : {}).then((r) => r.data),
};
...
export const scheduleApi = {
  ...
  historyVersion: (version) =>
    api.get(`/schedule/history/${version}`).then((r) => r.data),
};

export const replanApi = {
  companyDelay: (payload) =>
    api.post("/replan/company-delay", payload).then((r) => r.data),
  panelDrop: (payload) =>
    api.post("/replan/panel-drop", payload).then((r) => r.data),
  studentWithdraw: (payload) =>
    api.post("/replan/student-withdraw", payload).then((r) => r.data),
  roomUnavailable: (payload) =>
    api.post("/replan/room-unavailable", payload).then((r) => r.data),
};
